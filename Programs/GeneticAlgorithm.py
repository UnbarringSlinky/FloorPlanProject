...
def evaluate(individual):
    ...
    illegal_placement_penalty_value = 100000000  # Large penalty value to prevent any cameras from being inside walls or too far from walls.
...
def relocate_camera_to_radial_gap(camera, center, largest_radial_gap_angle, minimum_distance=90, step=10, max_distance=1000):
    ...
    while distance <= max_distance:
        ...
        if not is_camera_inside_obstacle(new_position, obstacles) and is_line_of_sight_clear(new_position, max_position, obstacles):
            # Update camera position
            # Update camera direction (facing away from the center)
            new_direction = (largest_radial_gap_angle + 180) % 360
...
def main():
    ...
    data = model.predict("C:\\Users\\ryanl\\OneDrive\\Desktop\\FloorPlanProject\\Data\\fp1.jpg", confidence=1).json()
    ...
    for gen in range(MAX_GENERATIONS):
        ...
        elite_individual = tools.selBest(population, 1)[0]
        ...
        if largest_gap_direction is not None:
            relocate_camera_to_radial_gap(elite_individual[least_impactful_camera_index], center, largest_gap_direction)
